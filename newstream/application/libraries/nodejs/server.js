...
io.sockets.on('connection', function(socket) {
	...
	if ( clients[socket.id] == undefined ) {
		...
		clients[socket.id]['stream'] = News.find({date:{'$gt':currentTime}}).tailable().stream();
	}

	socket.on('changeLanguage', function(lang){
		console.log(lang);
		if ( lang != undefined ) {
			clients[socket.id]['stream']._cursor.selector = {language:'Swedish'};
			clients[socket.id]['stream'].resume();
			console.log(clients[socket.id]['stream']._cursor.selector);
		}
	});

	clients[socket.id]['stream'].on('data', function(data) {
		clients[socket.id]['articles'].push(data);
		socket.emit('newArticle');
	});
	
	clients[socket.id]['stream'].on('error', function(err) {
		console.log(err);
	});	
